...
```{r}
# starting ag -5
ags <- c("hsa-miR-1" = -5 ,"hsa-miR-155-5p" = -5,"hsa-miR-124-3p" = -5,
         "hsa-miR-7-5p" = - 5,"hsa-miR-137" = -5,"hsa-miR-139-5p" = -5,
         "hsa-miR-143-3p" = -5 ,"hsa-miR-144-3p" = -5,"hsa-miR-153-3p" = -5,
         "hsa-miR-182-5p" = -5,"hsa-miR-199a-5p" = -5,"hsa-miR-204-5p" = -5,
         "hsa-miR-205-5p" = -5,"hsa-miR-216b-5p" = -5,"hsa-miR-223-3p" = -5,"cel-lsy-6" = -5) 
...
ob <- domnorm(read.delim("../data/McGeary_2019_raw/GSE140217_HeLa_transfection_logtpm_batchnormalized.txt", row.names=1))

# Exclude let7
ob <- ob[,colnames(ob)[colnames(ob) != "let7"]]
m[["hsa-let-7a"]] <- NULL
...
op$par
saveRDS(op,"opt_ags_McGearyLin_r2_hela.rds")
```
...
op <- readRDS("opt_ags_McGearyLin_r2_hela.rds")
...
ggsave("hela_corr_opt_ag_McGearyLin.png", width = 7)
