...
```{r}
miRNA_seqs <- read.xlsx("../data/aav1741_DataS1.xlsx", rows=131:142, colNames = FALSE)
miRNA_seqs_pass <- read.xlsx("../data/aav1741_DataS1.xlsx", rows=144:155, colNames = FALSE)
...
```{r}
g <- rtracklayer::import.gff("../data/McGeary_2019_raw/GSE140218_HEK293_transcriptannotations.gtf")
```
...
write.table(df_transcripts, "hek_transcripts_anB.txt", 
            sep="\t", quote=F, row.names = F)

orf_utr3 <- df_transcripts$orf_utr3
names(orf_utr3) <- df_transcripts$transcript
writeXStringSet(DNAStringSet(orf_utr3),"hek_orf_utr3_anB.fa")
