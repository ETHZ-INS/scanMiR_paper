...
aggAll_2 <- function(m, params=c(), ags=NULL){
  ...
  res <- bplapply(setNames(names(m),names(m)), BPPARAM=MulticoreParam(length(m)),
    #lapply(setNames(names(m),names(m)),
                  FUN=function(x){
...
feature_files <- list.files("/mnt/schratt/scanMiR_paper_scripts_old/Parameter_Optimization/data/McGeary_2019_processed/HEK_Features_McGeary_Scan/", full.names = T)
mirs_full <- gsub("\\.txt", "", list.files("/mnt/schratt/scanMiR_paper_scripts_old/Parameter_Optimization/data/McGeary_2019_processed/HEK_Features_McGeary_Scan/", full.names = F))
...
df4 <- readRDS("../../../Targetscan_Correlation/Run_Targetscan/Targetscan_corr_hek.rds")
df4$type <- "Targetscan"
df4$condition <- NULL
