...
p3 <- ggplot(Hoxb8_df, aes(x = start, y = -`logKd`)) + 
        ...
        ggtitle("Hoxb8 - miR-196a-5p") + xlab("3'UTR Sequence") + ylab(bquote("-"*logK[D])) + 
        theme_light() +
...
p5 <- ggplot(CDR1_df, aes(x = start, y = -`logKd`)) + 
        ...
        ggtitle("circular CDR1as") + xlab("Sequence") + ylab(bquote("-"*logK[D])) + 
...
p7 <- ggplot(Cyr_df, aes(x = start, y = -`logKd`)) + 
        ...
        ggtitle("Cyrano - miR-7-5p") + xlab("Sequence") + ylab(bquote("-"*logK[D]~"(>1)")) +
        theme_light() + ylim(1,6.5) +
        theme(
                panel.border =element_rect(color="black", fill=NA),
                text = element_text(size=12),
...
p9 <- ggplot(Nrep_df, aes(x = start, y = -`logKd`)) + 
        ...
        scale_color_manual(values = type_cols1) + 
        #geom_text(label = Cyr_df$type,nudge_y = -0.4) +
        ggtitle("Nrep - miR-29b-3p") + xlab("Sequence") + ylab(bquote("-"*logK[D])) + 
        theme_light() + ylim(1,6.5) +
        theme(
                panel.border =element_rect(color="black", fill=NA),
                text = element_text(size=12),
...
ppb <- plot_grid(bl,p4,bl,ncol = 3,labels = c("","E",""), rel_widths = c(1,5,1))
...
pp
ggsave("Figure_4.pdf",plot = pp, width = 8.5, height = 7,  bg = "white")
ggsave("Figure_4.png",plot = pp, width = 8.5, height = 7,  bg = "white")
```
...
# legend <- get_legend(
#   # create some space to the left of the legend
#   p5 + theme(legend.box.margin = margin(0, 8, 0, 0))
# )
# 
...
#                  nrow=2, rel_heights = c(6,2), labels = c("C","D")) + theme(plot.margin = unit(c(0,3,0,0), "lines"))
# 
...
# 
# pp4 <- plot_grid(p9 + theme(legend.position = "none"),
#                  p10,nrow=2, rel_heights = c(6,2), labels = c("G","H")) + theme(plot.margin = unit(c(0,3,0,0), "lines"))
# 
# 
# pp_a <- plot_grid(pp1,pp2,ggplot() + theme_void(),
#                 ncol = 3, labels = c(""),rel_widths = c(1,1,0.3), scale = 0.95)
# 
# pp_b <- plot_grid(pp3,pp4,legend,
#                 ncol = 3, rel_widths = c(1,1,0.3), labels = c(""), scale = 0.95)
# 
# pp <- plot_grid(pp_a,pp_b,ncol = 1)
# 
# 
# 
# ggsave("Figure_4.pdf",plot = pp, width = 13, height = 8, bg = "white")
# ggsave("Figure_4.png",plot = pp, width = 13, height = 8, bg = "white")
```
